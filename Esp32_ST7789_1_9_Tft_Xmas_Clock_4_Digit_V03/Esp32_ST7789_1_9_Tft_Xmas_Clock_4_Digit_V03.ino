/*
  This sketch displays a clock with 'Christmas' clock face on the 320*170 display
  of the ESP32 with ST7789 1.9 inch TFT display.
  ...
  see folder: more_faces | Christmas
  BMPs are 8 Color Depth and the original size is 135 x 240 pixels
*/

/*
Version Management
22.11.2025 V03 Just 4 digits but an animated seconds bar
22.11.2025 V02 unchanged main sketch but this version is using xmas-style clock faces
...
const char* PROGRAM_VERSION = "ESP32 ST7789 Xmas Clock 4 Digit V03";
const char* PROGRAM_TITLE = "ESP32 Xmas Clock";
...
const uint16_t DISPLAY_BACKGROUND_COLOR = TFT_WHITE;
const uint16_t DISPLAY_TITLE_COLOR = TFT_RED;
const uint16_t DISPLAY_SECONDS_BAR_COLOR = TFT_DARKGREEN;

const uint16_t CLOCK_FACE_WIDTH = 80;
const uint16_t CLOCK_FACE_HEIGHT = 142;

const uint16_t TITLE_POS_Y = 10;
const uint16_t CLOCK_POS_X = 0;
const uint16_t CLOCK_POS_Y = 10;
const uint16_t DIGITS_WIDTH = CLOCK_FACE_WIDTH;
const uint16_t CALENDER_POS_Y = 140;
const uint16_t SECONDS_BAR_POS_X = 9;
const uint16_t SECONDS_BAR_POS_Y = 150;
const uint16_t SECONDS_BAR_WIDTH = 302;
const uint16_t SECONDS_BAR_HEIGHT = 10;
...
void updateSecondsBar(uint8_t seconds) {
  if (seconds == 0) {
    tft.fillRect(SECONDS_BAR_POS_X, SECONDS_BAR_POS_Y, SECONDS_BAR_WIDTH, SECONDS_BAR_HEIGHT, DISPLAY_BACKGROUND_COLOR);
    tft.drawRect(SECONDS_BAR_POS_X, SECONDS_BAR_POS_Y, SECONDS_BAR_WIDTH, SECONDS_BAR_HEIGHT, DISPLAY_SECONDS_BAR_COLOR);
  } else {
    tft.drawRect(SECONDS_BAR_POS_X, SECONDS_BAR_POS_Y, SECONDS_BAR_WIDTH, SECONDS_BAR_HEIGHT, DISPLAY_SECONDS_BAR_COLOR);
    tft.fillRect(SECONDS_BAR_POS_X, SECONDS_BAR_POS_Y, seconds * 5, SECONDS_BAR_HEIGHT, DISPLAY_SECONDS_BAR_COLOR);
  }
}
